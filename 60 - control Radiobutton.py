...
class Aplicacion:
    def __init__(self):
        ...
        self.radio1=tk.Radiobutton(self.ventana1,text="Hombre", variable=self.seleccion, value=1)
    ...
    def mostrarseleccionado(self):
        if self.seleccion.get()==1:
            self.label1.configure(text="opcion seleccionada=Hombre")
        if self.seleccion.get()==2:
            self.label1.configure(text="opcion seleccionada=Mujer")
        
        # Cierra la ventana después de 2 segundos (2000 ms) para continuar con el siguiente código
